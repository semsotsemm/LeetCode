<h1 align="center">🚀 LeetCode Solutions</h1>

<p align="center">
    Welcome to my <strong>LeetCode Solutions Repository</strong>!  <br>
    Here, I document my approach to solving coding challenges, including problem descriptions, examples, and implementations.<br>
  🔗 My LeetCode Profile <br>
  Want to check out more of my LeetCode solutions? Visit my profile:  <br>
  [LeetCode - semsotsemm]: https://leetcode.com/u/4qkkr1pWqo/  <br>
</p>

---

## 🏆 Solved Problems
### 🟢Easy Problems  
🟢[Two Sum](Easy/Two_Sum)  
🟢[Divisible and Non-divisible Sums Difference](Easy/Divisible_and_Non-divisible_Sums_Difference)    
🟢[Palindrome Number](Easy/Palindrome_Number)    
🟢[Longest Common Prefix](Easy/Longest_Common_Prefix)  
🟢[Valid Parentheses](Easy/Valid_Parentheses)  
🟢[Find the Index of the First Occurrence in String](Easy/Find_the_Index_of_the_First_Occurrence_in_a_String)  

### 🟡Medium Problems  
🟡[Add Two Numbers](Medium/Add_Two_Numbers)  
🟡[Longest Substring Without Repeating Characters](Medium/Longest_Substring_Without_Repeating_Characters)    
🟡[String to Integer(atoi)](Medium/String_to_Integer_(atoi))  

### 🔴Hard Problems  
🔴[Maximizing nodes after merging two trees](Hard/Maximizing_nodes_after_merging_two_trees)  
🔴[Candy](Hard/Candy)  

---

## 🚀 How to Use
1. Clone the repository:
   ```bash
   git clone https://github.com/semsotsemm/LeetCode.git
